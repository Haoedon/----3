...
def get_key_bits() -> str:
    """Запрашивает ключ и возвращает 64 бита."""
    print("\n" + "=" * 70)
    print("ВВОД КЛЮЧА")
    print("=" * 70)
    ...
    if choice == "1":
        key = input("64-битный ключ: ").strip()
        if len(key) != 64 or not all(c in '01' for c in key):
            print("Ошибка: требуется ровно 64 символа 0 или 1")
            sys.exit(1)
        print(f"\nКлюч (биты): {key}")
        return key
...
def generate_gamma(key_bits: str, frame_num: int) -> list:
    ...
    for i in range(64):
        ...
        # Сохраняем старые состояния для вывода
        old_r1 = r1.copy()
        old_r2 = r2.copy()
        old_r3 = r3.copy()

        # Вычисляем новые биты
        new_r1 = r1[0] ^ bit
        new_r2 = r2[0] ^ bit
        new_r3 = r3[0] ^ bit
    ...
    for step in range(100):
        ...
        if step < 5 or step >= 95:  # Показываем только первые и последние 5 тактов
            ...
            # Пропускаем вывод для промежуточных тактов, но выполняем вычисления
            if r1[8] == majority:
    ...
    for i in range(114):
        ...
        if i < 10 or i >= 104:  # Показываем первые и последние 10 бит
            print(
            ...
            # Выполняем вычисления без вывода
            if r1[8] == majority:
                r1 = r1[1:] + [r1[13] ^ r1[16] ^ r1[17] ^ r1[18]]
            else:
                r1 = r1[1:] + [r1[18]]
